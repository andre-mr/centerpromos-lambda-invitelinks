...
describe("Lambda Handler Integration Tests", () => {
  const TEST_ACCOUNT = process.env.TEST_ACCOUNT?.toLowerCase();
  const TEST_CAMPAIGN = process.env.TEST_CAMPAIGN;
  const TEST_CATEGORY = process.env.TEST_CATEGORY;

  beforeAll(() => {
    // if (!TEST_ACCOUNT) {
    //   throw new Error("TEST_ACCOUNT must be defined in the .env file");
    // }
    if (!TEST_CAMPAIGN) {
  ...
  test("should redirect when accessing account and campaign only", async () => {
    const mockEvent = {
      rawPath: TEST_ACCOUNT ? `/:${TEST_ACCOUNT}/${TEST_CAMPAIGN}` : `/${TEST_CAMPAIGN}`,
      credentials,
    };
  ...
  test("should redirect when accessing campaign with category", async () => {
    const mockEvent = {
      rawPath: TEST_ACCOUNT
        ? `/:${TEST_ACCOUNT}/${TEST_CAMPAIGN}/${TEST_CATEGORY}`
        : `/${TEST_CAMPAIGN}/${TEST_CATEGORY}`,
      credentials,
    };
